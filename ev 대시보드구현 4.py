...
file_url = 'https://drive.google.com/uc?id=1A6ovFwPPOKQj_GcEBFfNZxmqelJsWATD'
output = 'Overview.xlsx'
...
# Google Drive에서 파일 다운로드
dfAA_url = "https://drive.google.com/uc?id=1b21PX_CKRldandWKRRPsjUN7ms-lwUzO"
dfBB_url = "https://drive.google.com/uc?id=112DAx-i1_1Wo3Ign_tiC293RAyHsvNyW"
dfAA_output = "dfAA.csv"
dfBB_output = "dfBB.csv"

# dfAA 파일 다운로드
if not os.path.exists(dfAA_output):
    gdown.download(dfAA_url, dfAA_output, quiet=False)
else:
    print(f"✅ '{dfAA_output}' 파일이 이미 존재합니다. 다운로드 생략.")

# dfBB 파일 다운로드
if not os.path.exists(dfBB_output):
    gdown.download(dfBB_url, dfBB_output, quiet=False)
else:
    print(f"✅ '{dfBB_output}' 파일이 이미 존재합니다. 다운로드 생략.")

# 다운로드한 CSV 파일 로드
dfAA = pd.read_csv(dfAA_output)
dfBB = pd.read_csv(dfBB_output)

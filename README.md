## 🚗 전기차 센서 데이터 분석 및 예측 대시보드 (Electric Vehicle Sensor Data Analysis and Prediction Dashboard)

이 대시보드는 전기차의 주행 센서 데이터를 분석하여 주행 시나리오별 배터리 소모 패턴을 파악하고, 이를 기반으로 배터리 성능 예측 및 주행 가능 거리 예측 모델의 결과를 시각적으로 제공합니다. Streamlit 기반의 사용자 친화적인 인터페이스를 통해 전기차 데이터에 대한 이해를 돕고 분석 결과를 쉽게 확인할 수 있도록 설계되었습니다.



## 🔧 주요 기능

주행 시나리오별 배터리 소모 분석: 다양한 주행 환경 및 패턴에 따른 배터리 소모 특성 시각화
배터리 성능 예측: 누적 주행 데이터 기반 배터리 상태 및 성능 변화 예측 모델 결과 제시
주행 거리 예측: 주행 데이터를 활용한 회귀 모델을 통해 현재 상태에서의 주행 가능 거리 예측
주요 데이터 시각화: 배터리 전압, 전류, 온도, 속도 등 핵심 센서 데이터의 트렌드 및 분포 분석



## 🧪 대상 데이터 및 모델

대상 데이터: 실제 전기차 주행 센서 데이터, 배터리 관리 시스템(BMS) 데이터 등
사용 모델: 주행 시나리오별 분석 기법, 배터리 성능 예측 모델, 주행 거리 예측을 위한 회귀 모델 등
데이터 항목: 배터리 충방전 상태, 주행 속도, 외부/내부 온도, 총 주행 거리 등 주요 센서 변수 포함
